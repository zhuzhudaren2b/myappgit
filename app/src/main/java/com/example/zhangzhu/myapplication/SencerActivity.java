...
/**
 * Created by zhangzhu on 2018/1/10.
 */

public class SencerActivity extends Activity implements SensorEventListener, View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.another_right_fragment);
        ...
        localWakeLock = this.localPowerManager.newWakeLock(getWakeLock(), "MyPower");//第一个参数为电源锁级别，第二个是日志tag
    }

    public int getWakeLock(){
        int mLock = 0;
        try {
            mLock = PowerManager.class.getField("PROXIMITY_SCREEN_OFF_WAKE_LOCK").getInt(null);
            System.out.println("mLock is : "+mLock);
        ...
        if (its != null && event.sensor.getType() == Sensor.TYPE_PROXIMITY) {
            ...
            if (its[0] == 0.0) {// 贴近手机
                ...
                if (localWakeLock.isHeld()) {
                    Log.d(TAG,"localWakeLock"+ localWakeLock +" is held11111");
                    return;
                } else{
                ...
                if (localWakeLock.isHeld()) {
                    Log.d(TAG,"localWakeLock"+ localWakeLock +" is held22222");
                    return;
                } else{
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.tv:
                localWakeLock = this.localPowerManager.newWakeLock(getWakeLock(), "MyPower");
        }
    }
}
